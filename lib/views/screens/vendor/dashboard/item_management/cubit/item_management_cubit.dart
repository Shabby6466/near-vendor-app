...
class ItemManagementCubit extends Cubit<ItemManagementState> {
  ...
  Future<void> createItem(CreateItemInput input) async {
    emit(ItemActionLoading());
    final response = await _itemServices.createItem(input);
    if (response.success) {
  ...
  Future<void> updateItem(UpdateItemInput input) async {
    emit(ItemActionLoading());
    final response = await _itemServices.updateItem(input);
    if (response.success) {
      emit(ItemActionSuccess(response.message));
      fetchItems();
    } else {
